...
def setup_external_package(package, external_type):
    spec = spack.spec.Spec(package)
    if external_type == "paths":
        external_location = "path/to/%s" % spec.name
    else:
        external_location = spec.name
    return ExternalPackage(spec, False, external_type, external_location)
...
class ExternalCmdTest(MockPackagesTest):
    ...
    def test_external_cmd_add(self):
        """
        Tests full function path from user arguments to addition of entries to
        packages.yaml
        """
        self.make_fake_install_path("external_package/2.0", "externalpackage")
        args = MockArgs("externalpackage@2.0%gcc@4.5.0",
                        self.external_package_path)
        spack.cmd.external.external_add(args)
        packages_config = get_packages_config_file()
        self.assertTrue("externalpackage" in packages_config.keys())
