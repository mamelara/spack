...
def external_list(args):
    ...
    for package_name in all_packages.keys():
        if package_name == "all":
            continue
        package_entry = packages_yaml.get_package(package_name)
        tty.hline(colorize(package_entry._package_name), char='-')
        for spec, location in package_entry.specs_section().iteritems():
            print spec, location
